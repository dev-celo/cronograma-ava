...
export default function CronogramaApp() {
  ...
  const handleAddMaterial = (material) => setMateriais([...materiais, material]);

  const handleFavorito = (link) => {
    if (!favoritos.find((f) => f.u === link.u)) setFavoritos([...favoritos, link]);
  };
  ...
  const exportarJSON = () => {
    const blob = new Blob([JSON.stringify({ anotacoes, materiais, favoritos, progresso }, null, 2)], { type: "application/json" });
  ...
  return (
    <div className="flex h-screen bg-gradient-to-r from-blue-50 to-blue-100">
        ...
        {Object.keys(data.agenda_semanal).map((d) => (
          <button
            ...
            className={`block text-left w-full p-3 rounded-xl mb-2 transition ${
              diaSelecionado === d ? "bg-blue-600 text-white shadow-md" : "bg-gray-100 hover:bg-blue-200"
            }`}
          >
      ...
      <main className="flex-1 p-6 overflow-y-auto">
        <h1 className="text-3xl font-extrabold mb-6 text-blue-700">{dia.nome}</h1>
        {dia.blocos &&
          dia.blocos.map((bloco, idx) => (
            <div key={idx} className="mb-6 bg-white rounded-2xl shadow-lg p-6 hover:shadow-xl transition">
              ...
              <ul className="mb-2 space-y-1">
                {bloco.links.map((link, i) => (
                  <li key={i} className="flex items-center gap-2">
                    <a href={link.u} target="_blank" rel="noreferrer" className="text-blue-600 underline">
                      {link.t}
                    </a>
                    <button
        ...
        <div className="bg-white rounded-2xl shadow-lg p-6 mb-6">
          ...
          <ul className="mb-2 space-y-1">
            {(anotacoes[diaSelecionado] || []).map((n, i) => (
              <li key={i} className="text-sm bg-gray-50 p-2 rounded">{n}</li>
            ))}
          </ul>
          <input
            type="text"
      ...
      <aside className="w-80 bg-white shadow-xl p-6 rounded-l-2xl overflow-y-auto">
        ...
        <h3 className="font-semibold text-gray-700 mt-4">🔥 Prioridades ({diaSelecionado})</h3>
        <ul className="text-sm mb-4 space-y-1">
          {data.prioridades_por_disciplina[dia.nome.split(" ")[0]]?.map((p, i) => (
            <li key={i}>{p}</li>
          ))}
        </ul>

        <h3 className="font-semibold text-gray-700 mt-4">📝 Observações Globais</h3>
        <ul className="text-sm mb-4 space-y-1">
